...
def codex_prompt_builder(request: dict, output_root: Path) -> str:
    return build_generic_prompt(request, output_root, "Codex")


def codex_cli_invoker(
    prompt: str,
    output_root: Path,
    model: str | None,
    ...
    cmd = ["codex", "exec", prompt, "-s", "workspace-write"]
    if model:
        cmd.extend(["-m", model])
...
def main() -> int:
    ...
    if args.watch:
        ...
        return watch_generic_requests(
            ...
            pid_file_path=Path(args.pid_file) if args.pid_file else (base_dir / "leases" / "codex.pid"),
            prompt_builder=codex_prompt_builder,
            cli_invoker=codex_cli_invoker,
        )
